          ...
          <div class="row mb-2">
            <div class="col-12">
              <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4>Currencies</h4>
                <Breadcrumb :items="[ROUTE_ADMIN_CURRENCY()]" />
              </div>
        ...
        <form @submit.prevent="store(faq)">
          <div class="row ">
            <div class="col-md-12 py-3 mb-3 border-bottom">
              <h5 class="float-left" v-if="update_mode">Update Currency</h5>
              <h5 class="float-left" v-else>Add Currency</h5>
              <h5 class="float-right" @click="closeModal"><i class="pi pi-times"></i></h5>
            </div>
            <FormControlInput :col_md="12" v-model="faq.name" label="Name of Currency" placeholder="US Dollar" />
            <FormControlSelect2 :options="CURRENCY_CODE_OPTIONS" :col_md="12" v-model="faq.code" label="Code"
              placeholder="USD" />
            <FormControlInput :col_md="12" v-model="faq.format" label="Format" placeholder="Format" />
...
import {
  CURRENCY_CODES,
  FORM_DATA,
  notify, ROUTE_ADMIN_CURRENCY, ROUTE_ADMIN_FAQ,
} from "../../../constants.js";
...
export default {
  ...
  methods: {
    ...
    async list(line=false) {
      ...
      await this.axios.post(`admin/currencies/fetch`,this.table).then(({ data }) => {
    ...
    async store(data) {
      ...
      await this.axios.post(`admin/currencies/store`,FORM_DATA(data)).then(({ data }) => {
        this.list();
        this.closeModal();
        this.createMode();
        notify(data.message);
    ...
    _delete() {
      this.axios.post("admin/currencies/delete", { id:this.delete_id }).then((response) => {
